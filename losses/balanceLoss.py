...
class BalanceLoss(nn.Module):
    ...
    def forward(self, cls_input, center_rate):
        ...
        center_num = torch.tensor(self.create_hanning_mask(self.center_R)).unsqueeze(0).repeat(int(target.size(0)), 1,
                                                                                               1)  # 创建汉宁窗口 并且
...
class LocLoss(nn.Module):
    def __init__(self, device=None, opt=None):
        ...
        self.center_R = opt.pos_label
        self.opt = opt

    def forward(self, cls_input, loc_input, center_rate):

        # cls_part = cls_input.reshape(cls_input.shape[0], -1)
        # topk_indices = torch.topk(cls_part, self.opt.pos_num , dim=1).indices
        # label_mask = torch.zeros_like(cls_part, device="cuda:0", dtype=bool)
        # for i in range(topk_indices.shape[0]):
        #     label_mask[i][topk_indices[i]] = True

        target, label_mask = self.create_labels(cls_input.size(), center_rate,self.center_R)  # 创建正样本centerR 的范围，超过这个范围全部都是负样本或者忽略样本
        label_mask = label_mask.reshape(cls_input.shape[0], -1)  # 将label结果抽成一行向量

        # target2, label_mask2 = self.create_labels(cls_input.size(), center_rate,77)  # 创建正样本centerR 的范围，超过这个范围全部都是负样本或者忽略样本
        # label_mask2 = label_mask2.reshape(cls_input.shape[0], -1)  # 将label结果抽成一行向量
        # label_mask = label_mask2-label_mask

        cls_part = cls_input.reshape(cls_input.shape[0], -1)  # 将分类结果抽成一行向量
        topk_indices = torch.topk(cls_part, self.opt.pos_num, dim=1).indices  # 选取top10个参数(16,1)
        # label_mask = torch.zeros_like(cls_part, device="cuda:" + str(self.device_cuda),
        #                                  dtype=bool)  # 创建一个掩膜用于保存正样本的位置将其置为True
        # label_mask = label_mask.reshape(cls_input.shape[0], -1)  # 将label结果抽成一行向量
        for i in range(topk_indices.shape[0]):
            label_mask[i][topk_indices[i].cpu().numpy()] = label_mask[i][topk_indices[i].cpu().numpy()] + 1

        pos_mask = label_mask.reshape(-1)  # 掩膜用于保存正样本的位置将其置为True
        pos_mask = pos_mask[:] == 2
        bias_map = self.create_loc_bias(cls_input.size(), center_rate, opt=self.opt)  # 每个位置的调整参数
        bias_map = bias_map.reshape(-1, 2)  # reshape
        bias_pos = bias_map[pos_mask].reshape(-1)  # 提取正样本的实际调整参数
        # bias_pos_limit = (bias_pos+16.5)/33 # 提取正样本的实际调整参数
        bias_pos_x = bias_map[pos_mask][:, 0]
        bias_pos_y = bias_map[pos_mask][:, 1]
        res_pos = loc_input.permute(0, 2, 3, 1).contiguous().reshape(-1, 2)  # 调整模型输出维度
        res_pos_x = res_pos[pos_mask][:, 0]  # model out bias x
        res_pos_x_sig = torch.sigmoid(res_pos_x) * self.center_R - (self.center_R / 2)
        res_pos_y = res_pos[pos_mask][:, 1]  # model out bias y
        res_pos_y_sig = torch.sigmoid(res_pos_y) * self.center_R - (self.center_R / 2)
        res_pos = res_pos[pos_mask].reshape(-1)  # 提取正样本，模型输出的调整参数
        res_pos_sig = torch.sigmoid(res_pos) * self.center_R - (self.center_R / 2)
        if self.opt.loc_loss == "smooth_l1_loss":
            loc_loss = F.smooth_l1_loss(res_pos, bias_pos, reduction='mean')  # 计算损失
            # loc_loss = F.smooth_l1_loss(res_pos_sig, bias_pos, reduction='mean')  # 计算损失
            if len(res_pos) == 0:
                loc_loss = torch.tensor((0))
        elif self.opt.loc_loss == "BCE":  # 损失存在负值
            loss_x = torch.mean(self.BCELoss(res_pos_x, bias_pos_x))
            loss_y = torch.mean(self.BCELoss(res_pos_y, bias_pos_y))
            if len(res_pos_x) == 0:
                loss_x = loss_y = torch.tensor((0))
            loc_loss = loss_x + loss_y
        elif self.opt.loc_loss == "MSE":
            loss_x = torch.mean(self.MSELoss(res_pos_x, bias_pos_x))
            loss_y = torch.mean(self.MSELoss(res_pos_y, bias_pos_y))
            if len(res_pos_x) == 0:
                loss_x = loss_y = torch.tensor((0))
            loc_loss = loss_x + loss_y

        # for i ,num in enumerate(topk_indices):
        #     for pos_index in num:
        #         if label_mask[i][pos_index]==1:
        #             cls_topk_mask[i][pos_index] = True
        # pos_mask = cls_topk_mask.reshape(-1)  # 掩膜用于保存正样本的位置将其置为True
        # bias_map = self.create_loc_bias(cls_input.size(), center_rate)  # 每个位置的调整参数
    ...
    def create_loc_bias(self, size, center_rate, opt):
        b, c, h, w = size
        x = np.arange(h)
        y = np.arange(w)
    ...
    def create_labels(self, size, rate, center_R):
        ratex, ratey = rate
        labels = np.zeros(size)
        b, c, h, w = size
        X = ratex * (h - 1)
        ...
        for i in range(b):
            CenterX, CenterY = CenterXY[i]
            pad_right = pad_left = pad_top = pad_bottom = 0  # 防止centerR 超出边界
            ...
            x1, x2, y1, y2 = new_center[0] - center_R // 2, \
                             ...
                             new_center[1] + center_R // 2 + 1  # 为什么＋在创建标签时左闭右开
            label = new_label.copy()
            label[int(x1):int(x2), int(y1):int(y2)] = 1
            label_mask = new_label != -1
            new_label_out = label[label_mask].reshape(h, w)
...
class LossFunc(nn.Module):
    def __init__(self, center_R, neg_weight=15.0, device_cuda=None, opt=None):
        ...
        self.loc_loss = LocLoss(device=device_cuda, opt=opt)
        # self.loc_loss = 0
    ...
    def forward(self, input, center_rate):
        ...
        if loc_input is not None:
            ...
            # loc_loss = torch.tensor((0))
            # return cls_loss*(1-self.loc_wight), loc_loss*self.loc_wight
